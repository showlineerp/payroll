					...
					->addColumn('salary_details', function ($row) {
						...
						if ($row->allowances) {
							...
							foreach ($row->allowances as $allowance) {
								...
								if ($allowance['is_taxable']) {
									$taxable_allowances += $allowance['allowance_amount'] * ($allowance['deductible_amt'] / 100);
								}
							}
						} else {
							$allowance_total = 0;
						...
						if ($row->deductions) {
							...
							foreach ($row->deductions as $deduction) {
								...
								if ($deduction['is_taxable']) {
									$allowable_deductions += $deduction['deduction_amount'] * ($deduction['deductible_amt'] / 100);
								} else {
									$other_deductions += $deduction['deduction_amount'];
								}
							}
						}
						$zimra_deduction = $this->calculate_zimra($basic_salary, $taxable_allowances, $allowable_deductions, $other_deductions, $row->payment_type);
						Log::info('Deduction: ', $zimra_deduction);
						$deduction_total+= $zimra_deduction;
						//Basic Salary(Total)
						if ($row->payment_type == 'Hourly') {
						...
						if ($row->allowances) {
							$data .= "<div class='d-flex'>
									...
									<div class='ml-auto'>" . $allowance_total . "88</div>
								</div>";
						}
						//Total Salary
						$data .= "<div class='d-flex'>
